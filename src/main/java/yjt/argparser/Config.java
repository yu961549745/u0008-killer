...
public class Config extends GsonToString {
    ...
    public boolean searchRecursive = true;
    @Option(name = "-rh", usage = "replace hidden files")
    public boolean replaceHidden = false;
    @Option(name = "-c", usage = "charset of input file(s)")
    public String charset = "UTF8";
    @Option(name = "-ext", usage = "指定需要替换的文件的后缀名, 用 | 连接, 比如 'md' 或 'md|java', 默认替换所有文件")
    public String replaceFileExtensions = null;
    @Argument(usage = "root search folder")
